...
import com.boddi.honeycomb.sparkbee.config.ConfigurationManager
...
object ExtractorApp {
  ...
  def main(args: Array[String]): Unit = {
    val spark = SparkSession
      ...
      .config("spark.master", ConfigurationManager.getProperty("spark.master"))
      .getOrCreate()
    ...
    val jdbcConfigResource = this.getClass.getClassLoader().getResource("jdbc_conf.json")
    val jdbcConfDF = spark.read.json("file://"+jdbcConfigResource.getPath)
//    val jdbcConfDF = spark.read.json("/Users/guoyubo/workspace/honeycomb/spark-bee/src/main/resources/jdbc_conf.json")
    ...
    val tableConfigResource = this.getClass.getClassLoader().getResource("table_conf.json")
    val tableConfDF = spark.read.json("file://"+tableConfigResource.getPath)
    tableConfDF.show()
    ...
    val result = spark.sql("select BATCH_ID,FROM_TABLE_NAME,TABLE_OWNER,PRIMARY_KEY,TO_TABLE_NAME,MERGE_TYPE,JNDI_NAME from ETL_TABLE_CONF ").rdd.collect().foreach(e => {
      ...
      val tablePath = ConfigurationManager.getProperty("HADOOP_ODL_EXTERNAL_PATH") + e.getString(4)
      extractor.execute("jdbc", jdni._1, jdni._2, jdni._3, jdni._4,
        e.getString(1), e.getString(3), "1=1", e.getString(4), tablePath, dateStr)
      loader.execute(e.getString(4), "", tablePath, dateStr)
    })
